...
class CompleteDataGridView extends TPage
{
    ...
    public function __construct()
    {
        ...
        $name = new TEntry('name');
          $comente = new TEntry('comente');
            $birthdate = new TEntry('birthdate');
        $this->form->addFields( [new TLabel('Name:')], [$name] );
          $this->form->addFields( [new TLabel('Comente:')], [$comente] );
            $this->form->addFields( [new TLabel('Aniversario:')], [$birthdate] );
        ...
        $col_comente  = new TDataGridColumn('comente', 'Comente', 'left', '50%');
        $col_birthdate = new TDataGridColumn('birthdate', 'Aniversario', 'left', '30%');
        
        // assign the ordering actions
        $col_id->setAction(new TAction([$this, 'onReload']), ['order' => 'id']);
        $col_name->setAction(new TAction([$this, 'onReload']), ['order' => 'name']);
 $col_comente->setAction(new TAction([$this, 'onReload']), ['order' => 'comente']);
  $col_birthdate->setAction(new TAction([$this, 'onReload']), ['order' => 'birthdate']);
        // add the columns to the DataGrid
        $this->datagrid->addColumn($col_id);
        $this->datagrid->addColumn($col_name);
         $this->datagrid->addColumn($col_comente);
        $this->datagrid->addColumn($col_birthdate);
    ...
    function onSearch()
    {
        ...
        if (isset($data->name))
        {
            ...
            $filter = new TFilter('name', 'like', "%{$data->name}%");
                $filter = new TFilter('comente', 'like', "%{$data->comente}%");
                      $filter = new TFilter('birthdate', 'like', "%{$data->birthdate}%");
            // stores the filter in the session
            TSession::setValue('City_filter', $filter);
            TSession::setValue('City_name',   $data->name);
              TSession::setValue('City_comente',   $data->comente);
              TSession::setValue('City_birthdate',   $data->birthdate);
    ...
    public static function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('clientes'); // open a transaction with database
